...
const CustomTooltip = memo(({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white px-4 py-3 rounded-lg shadow-lg border border-gray-200">
      <p className="font-semibold text-gray-800 mb-1">{payload[0].payload.label}</p>
...
export const RamDistributionChart = memo(({ data }: RamDistributionChartProps) => {
  ...
  const { formattedData, totalDevices } = useMemo(() => {
    ...
    const total = data.reduce((sum, item) => sum + item.device_count, 0);
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-base">Distribution RAM (GB)</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="flex items-center justify-center h-64">
          <div className="text-center">
            <MemoryStick className="h-12 w-12 text-gray-300 mx-auto mb-3" />
