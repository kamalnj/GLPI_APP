...
const CustomTooltip = memo(({ active, payload }: any) => {
    ...
    return (
        <div className="rounded border border-gray-200 bg-white px-3 py-2 shadow-sm">
            <p className="text-sm font-medium">{data.groupe}</p>
            <p className="font-semibold text-blue-600">
                {data.software_count} logiciels
            </p>
            <p className="mt-1 text-xs text-gray-600">
                {data.device_count} appareils
            </p>
        </div>
    );
});
...
export const TopGroupsSoftwaresChart = memo(
    ({ data }: TopGroupsSoftwaresChartProps) => {
        // Memoize calculations
        const { totalSoftware, totalDevices } = useMemo(
            () => ({
                totalSoftware:
                    data?.reduce((sum, item) => sum + item.software_count, 0) ||
                    0,
                totalDevices:
                    data?.reduce((sum, item) => sum + item.device_count, 0) ||
                    0,
            }),
            [data],
        );
