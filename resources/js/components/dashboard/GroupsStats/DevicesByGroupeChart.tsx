...
const CustomTooltip = memo(({ active, payload }: any) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded px-3 py-2 shadow-sm">
      ...
      <p className="text-orange-600 font-semibold">{payload[0].value} appareils</p>
    </div>
  );
});
...
export const LowDiskDevicesByGroupeChart = memo(({ data }: LowDiskDevicesByGroupeChartProps) => {
  // Memoize calculations
  const totalDevices = useMemo(() => 
    data?.reduce((sum, item) => sum + item.low_disk_devices, 0) || 0,
    [data]
  );
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <span className="text-2xl font-bold text-orange-600">{totalDevices}</span>
        </div>
      </CardHeader>
      <CardContent>
