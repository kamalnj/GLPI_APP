...
const DISK_SEGMENTS = [
    { key: 'critical' as const, label: 'Critique', sub: '< 10% libre',  color: '#E24B4A', textClass: 'text-red-700 dark:text-red-300',   bgClass: 'bg-red-50 dark:bg-red-900/30',   dotClass: 'bg-red-500'   },
    { key: 'alert'    as const, label: 'Alerte',   sub: '10–20% libre', color: '#EF9F27', textClass: 'text-amber-700 dark:text-amber-300', bgClass: 'bg-amber-50 dark:bg-amber-900/30', dotClass: 'bg-amber-400' },
    { key: 'normal'   as const, label: 'Normal',   sub: '> 20% libre',  color: '#639922', textClass: 'text-green-700 dark:text-green-300', bgClass: 'bg-green-50 dark:bg-green-900/30', dotClass: 'bg-green-600' },
];
 
const CustomTooltip = ({ active, payload }: any) => {
    if (!active || !payload?.length) return null;
    const { name, value, payload: p } = payload[0];
    const seg = DISK_SEGMENTS.find(s => s.label === name);
    return (
...
export default memo(function DiskPieChart({ stats }: { stats: DiskStats }) {
    const { critical, alert, normal, total } = stats;
    const chartData = [
        { name: 'Critique', value: critical, sub: '< 10% libre',  pct: Math.round(critical / total * 100) },
        { name: 'Alerte',   value: alert,    sub: '10–20% libre', pct: Math.round(alert    / total * 100) },
        { name: 'Normal',   value: normal,   sub: '> 20% libre',  pct: Math.round(normal   / total * 100) },
    ];
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            <div className="flex items-center justify-between mb-4">
                <div>
                    <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Espace disque</h3>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie data={chartData} cx="50%" cy="50%" innerRadius={58} outerRadius={82} paddingAngle={2} dataKey="value" strokeWidth={0} isAnimationActive={false}>
                                {chartData.map((_, i) => <Cell key={i} fill={DISK_SEGMENTS[i].color} />)}
                            </Pie>
